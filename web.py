#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
Created on Sat Apr 10 13:04:27 2021

@author: aly
"""
from owlready2 import *
#from rdflib import *
import json
import rdflib
from flask_cors import CORS

#onto = get_ontology("estilosR.owl").load()  ontologia vieja
onto = get_ontology("Estilos_de_Aprendizaje.owl").load()
graph = default_world.as_rdflib_graph()

g = rdflib.Graph()
g.parse("Estilos_de_Aprendizaje.owl")


from flask import Flask, request

app = Flask(__name__)
CORS(app)
@app.route('/')
def ontology_page():
    html  = """<html><body>"""
    html += """<h2>Consultas disponibles</h2>""" 
  #  html += """<h2>'%s' ontology</h2>""" % onto.base_iri
  #  html += """<h3>Root classes</h3>"""
  #  for Class in Thing.subclasses():
  #      html += """<p>>%s</p>""" % Class.name
   #     html += """<p>>%s</p>""" % len(g)
   
    
    html += """<div>
                <p>Seleccione su consulta:</p>
  <form action="/result">
    <input type="radio" id="1" name="c" value="1">
  <label for="1">Consulta 1</label><br>
  <input type="radio" id="2" name="c" value="2">
  <label for="2">Consulta 2</label><br>
  <input type="radio" id="3" name="c" value="3">
  <label for="3">Consulta 3 Saludos hola</label>
  </p>
<p><input type="submit"></p>
    </form>
    </div>""" 
    
    return html

@app.route('/result')
def consulta():
  consul = request.args.get("c"," ")
  ruta = []
  html = ""
  #sync_reasoner_hermit()
    
  if consul == "2" :
       # html = """<p>>%s</p>""" % onto.Mat_Vis_Glo_M.instances()
        qres = g.query("""
        PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>
        PREFIX owl: <http://www.w3.org/2002/07/owl#>
        PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>
        PREFIX xsd: <http://www.w3.org/2001/XMLSchema#>
        PREFIX  :<http://www.semanticweb.org/miguelangel/ontologies/2022/estilos#>
        SELECT   ?x ?d WHERE { 
          ?x :se_identifica_con_e_a  :visual_secuencial_apropiado.
          ?x :direccion_web ?d.
        }""")
        unique_category = []
        for row in qres:
           category = ("%s %s" % row).rsplit('/',2)[-1]
           unique_category.append(category)
        archivo = json.dumps(unique_category)
        html = archivo
	
  elif consul == "1" :
        qres = g.query("""
        PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>
        PREFIX owl: <http://www.w3.org/2002/07/owl#>
        PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>
        PREFIX xsd: <http://www.w3.org/2001/XMLSchema#>
        PREFIX  :<http://www.semanticweb.org/miguelangel/ontologies/2022/estilos#>
        SELECT   ?x ?d ?r WHERE { 
          ?x :se_identifica_con_e_a  :visual_secuencial_moderado.
          ?x :direccion_web ?d.
	  ?x :nivel ?r.
        }""")
        unique_category = []
        for row in qres:
           category = ("%s %s %s" % row).rsplit('/',2)[-1]
           unique_category.append(category)
        archivo = json.dumps(unique_category)
        html = archivo

  elif consul == "3" :
        qres = g.query("""
        PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>
        PREFIX owl: <http://www.w3.org/2002/07/owl#>
        PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>
        PREFIX xsd: <http://www.w3.org/2001/XMLSchema#>
        PREFIX  :<http://www.semanticweb.org/miguelangel/ontologies/2022/estilos#>
        SELECT   ?x ?d ?r WHERE { 
          ?x :se_identifica_con_e_a  :visual_secuencial_fuerte.
          ?x :direccion_web ?d.
	  ?x :nivel ?r.
        }""")
        unique_category = []
        for row in qres:
           category = ("%s %s %s" % row).rsplit('/',2)[-1]
           unique_category.append(category)
        archivo = json.dumps(unique_category)
        html = archivo
  
  elif consul == "5" :
        qres = g.query("""
        PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>
        PREFIX owl: <http://www.w3.org/2002/07/owl#>
        PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>
        PREFIX xsd: <http://www.w3.org/2001/XMLSchema#>
        PREFIX  :<http://www.semanticweb.org/miguelangel/ontologies/2022/estilos#>
        SELECT   ?x ?d ?r WHERE { 
          ?x :se_identifica_con_e_a  :visual_global_apropiado.
          ?x :direccion_web ?d.
	  ?x :nivel ?r.
        }""")
        unique_category = []
        for row in qres:
           category = ("%s %s %s" % row).rsplit('/',2)[-1]
           unique_category.append(category)
        archivo = json.dumps(unique_category)
        html = archivo
	
  elif consul == "4" :
        qres = g.query("""
        PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>
        PREFIX owl: <http://www.w3.org/2002/07/owl#>
        PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>
        PREFIX xsd: <http://www.w3.org/2001/XMLSchema#>
        PREFIX  :<http://www.semanticweb.org/miguelangel/ontologies/2022/estilos#>
        SELECT   ?x ?d ?r WHERE { 
          ?x :se_identifica_con_e_a  :visual_global_moderado.
          ?x :direccion_web ?d.
	  ?x :nivel ?r.
        }""")
        unique_category = []
        for row in qres:
           category = ("%s %s %s" % row).rsplit('/',2)[-1]
           unique_category.append(category)
        archivo = json.dumps(unique_category)
        html = archivo
	
  elif consul == "6" :
        qres = g.query("""
        PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>
        PREFIX owl: <http://www.w3.org/2002/07/owl#>
        PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>
        PREFIX xsd: <http://www.w3.org/2001/XMLSchema#>
        PREFIX  :<http://www.semanticweb.org/miguelangel/ontologies/2022/estilos#>
        SELECT   ?x ?d ?r WHERE { 
          ?x :se_identifica_con_e_a  :visual_global_fuerte.
          ?x :direccion_web ?d.
	  ?x :nivel ?r.
        }""")
        unique_category = []
        for row in qres:
           category = ("%s %s %s" % row).rsplit('/',2)[-1]
           unique_category.append(category)
        archivo = json.dumps(unique_category)
        html = archivo
	
  elif consul == "7" :
        qres = g.query("""
        PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>
        PREFIX owl: <http://www.w3.org/2002/07/owl#>
        PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>
        PREFIX xsd: <http://www.w3.org/2001/XMLSchema#>
        PREFIX  :<http://www.semanticweb.org/miguelangel/ontologies/2022/estilos#>
        SELECT   ?x ?d ?r WHERE { 
          ?x :se_identifica_con_e_a  :verbal_secuencial_moderado.
          ?x :direccion_web ?d.
	  ?x :nivel ?r.
        }""")
        unique_category = []
        for row in qres:
           category = ("%s %s %s" % row).rsplit('/',2)[-1]
           unique_category.append(category)
        archivo = json.dumps(unique_category)
        html = archivo
	
  elif consul == "8" :
        qres = g.query("""
        PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>
        PREFIX owl: <http://www.w3.org/2002/07/owl#>
        PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>
        PREFIX xsd: <http://www.w3.org/2001/XMLSchema#>
        PREFIX  :<http://www.semanticweb.org/miguelangel/ontologies/2022/estilos#>
        SELECT   ?x ?d ?r WHERE { 
          ?x :se_identifica_con_e_a  :verbal_secuencial_apropiado.
          ?x :direccion_web ?d.
	  ?x :nivel ?r.
        }""")
        unique_category = []
        for row in qres:
           category = ("%s %s %s" % row).rsplit('/',2)[-1]
           unique_category.append(category)
        archivo = json.dumps(unique_category)
        html = archivo
	
  elif consul == "9" :
        qres = g.query("""
        PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>
        PREFIX owl: <http://www.w3.org/2002/07/owl#>
        PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>
        PREFIX xsd: <http://www.w3.org/2001/XMLSchema#>
        PREFIX  :<http://www.semanticweb.org/miguelangel/ontologies/2022/estilos#>
        SELECT   ?x ?d ?r WHERE { 
          ?x :se_identifica_con_e_a  :verbal_secuencial_fuerte.
          ?x :direccion_web ?d.
	  ?x :nivel ?r.
        }""")
        unique_category = []
        for row in qres:
           category = ("%s %s %s" % row).rsplit('/',2)[-1]
           unique_category.append(category)
        archivo = json.dumps(unique_category)
        html = archivo
	
  elif consul == "10" :
        qres = g.query("""
        PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>
        PREFIX owl: <http://www.w3.org/2002/07/owl#>
        PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>
        PREFIX xsd: <http://www.w3.org/2001/XMLSchema#>
        PREFIX  :<http://www.semanticweb.org/miguelangel/ontologies/2022/estilos#>
        SELECT   ?x ?d ?r WHERE { 
          ?x :se_identifica_con_e_a  :verbal_global_moderado.
          ?x :direccion_web ?d.
	  ?x :nivel ?r.
        }""")
        unique_category = []
        for row in qres:
           category = ("%s %s %s" % row).rsplit('/',2)[-1]
           unique_category.append(category)
        archivo = json.dumps(unique_category)
        html = archivo
	
  elif consul == "11" :
        qres = g.query("""
        PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>
        PREFIX owl: <http://www.w3.org/2002/07/owl#>
        PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>
        PREFIX xsd: <http://www.w3.org/2001/XMLSchema#>
        PREFIX  :<http://www.semanticweb.org/miguelangel/ontologies/2022/estilos#>
        SELECT   ?x ?d ?r WHERE { 
          ?x :se_identifica_con_e_a  :verbal_global_apropiado.
          ?x :direccion_web ?d.
	  ?x :nivel ?r.
        }""")
        unique_category = []
        for row in qres:
           category = ("%s %s %s" % row).rsplit('/',2)[-1]
           unique_category.append(category)
        archivo = json.dumps(unique_category)
        html = archivo
	
  elif consul == "12" :
        qres = g.query("""
        PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>
        PREFIX owl: <http://www.w3.org/2002/07/owl#>
        PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>
        PREFIX xsd: <http://www.w3.org/2001/XMLSchema#>
        PREFIX  :<http://www.semanticweb.org/miguelangel/ontologies/2022/estilos#>
        SELECT   ?x ?d ?r WHERE { 
          ?x :se_identifica_con_e_a  :verbal_global_fuerte.
          ?x :direccion_web ?d.
	  ?x :nivel ?r.
        }""")
        unique_category = []
        for row in qres:
           category = ("%s %s %s" % row).rsplit('/',2)[-1]
           unique_category.append(category)
        archivo = json.dumps(unique_category)
        html = archivo
	
  else:
         d = (list(graph.query_owlready("""
                              PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>
                              PREFIX owl: <http://www.w3.org/2002/07/owl#>
                              PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>
                              PREFIX xsd: <http://www.w3.org/2001/XMLSchema#>
                              PREFIX  :<http://www.semanticweb.org/miguelangel/ontologies/2022/estilos#>
                              SELECT distinct ?a WHERE {
                                  ?a estilo:se_relaciona_con estilo:verbal_secuencial_fuerte.
                                       }""")))
         
         
         for x in range(0,len(d)):
            st = str(d[x])
            st = st[1:-1]
            ruta.append(st)
         # str(d[x]) html += """<br>>%s</br>""" % str(d[x])
         archivo = json.dumps(ruta)
         html = archivo
    
  return html
           
  
  #  sync_reasoner_hermit()
  #  html = """<p>>%s</p>""" % onto.Mat_Vis_Glo_M.instances()


#import werkzeug.serving
#werkzeug.serving.run_simple("localhost", 5000, app)
port=os.environ["PORT"]
app.run('0.0.0.0',port, debug=True)
